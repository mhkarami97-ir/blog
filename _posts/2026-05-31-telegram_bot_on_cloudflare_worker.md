...
## امکانات بیشتر
بخش Bindings در فایل `wrangler.toml` امکان اتصال به سرویس‌های مختلف را فراهم می‌کند. برای مثال، اتصال به KV Storage یا R2 Storage می‌تواند برای ذخیره‌سازی داده‌ها یا فایل‌ها مورد استفاده قرار گیرد.  
امکان قرار دادن دامنه سفارشی نیز وجود دارد که می‌تواند به‌جای آدرس پیش‌فرض Worker استفاده شود. این کار با تنظیم DNS و پیکربندی در همان صفحه ورکر در Cloudflare انجام می‌شود.  